    ...
    <div class="rounded shadow-lg p-6 w-full z-10 bg-white dark:bg-neutral-900 text-gray-900 dark:text-gray-100">
        <form wire:submit.prevent="store">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                ...
                {{-- ✅ Campo Entregado por (Combobox modificado) --}}
                <div>
                    ...
                    <div
                        x-data="{
                            is_open: false,
                            active_index: -1,
                            selected_name: @entangle('searchEntregado').defer, // Mantiene el nombre visible
                            selectOption(id, name) {
                                @this.set('entregado_por_id', id);
                                this.selected_name = name;
                                @this.set('searchEntregado', ''); // Limpia el filtro interno
                                this.is_open = false;
                                this.active_index = -1;
                            },
                            navigate(direction) {
                        ...
                        class="relative"
                    >
                        {{-- BOTÓN/CAJA DE SELECCIÓN PRINCIPAL --}}
                        <button
                            type="button"
                            @click="is_open = !is_open; $nextTick(() => is_open && $refs.searchInput.focus())"
                            ...
                            <ul class="py-1" x-ref="listbox">
                                @if (count($this->responsables) > 0)
                                    @foreach ($this->responsables as $index => $r)
                                        <li
                                            id="entregado-item-{{ $index }}"
                                            wire:key="entregado-{{ $r->id }}"
                                    ...
                                    @endforeach
                                @else
                                    {{-- El mensaje de advertencia de "Escribe 3 o más" se elimina --}}
                                    <li class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400">
                                        No se encontraron responsables con la búsqueda.
                                    </li>
                                @endif
                            </ul>
                        </div>
                ...
                {{-- ✅ Campo Recibido por (Combobox modificado) --}}
                <div>
                    ...
                    <div
                        x-data="{
                            is_open: false,
                            active_index: -1,
                            selected_name: @entangle('searchRecibido').defer, // Mantiene el nombre visible
                            selectOption(id, name) {
                                @this.set('recibido_por_id', id);
                                this.selected_name = name;
                                @this.set('searchRecibido', ''); // Limpia el filtro interno
                                this.is_open = false;
                                this.active_index = -1;
                            },
                            navigate(direction) {
                        ...
                        <div x-show="is_open"
                            ...
                            <div class="p-2 border-b border-gray-200 dark:border-neutral-700 sticky top-0 bg-white dark:bg-neutral-800 z-40">
                                <input
                                    x-ref="searchInput"
                                    type="text"
                                    wire:model.live.debounce.300ms="searchRecibido"
                                    @keydown.escape.prevent="is_open = false"
                                    @keydown.arrow-down.prevent="navigate('down')"
                                    @keydown.arrow-up.prevent="navigate('up')"
                                    @keydown.enter.prevent="selectActive()"
                            ...
                            <ul class="py-1" x-ref="listbox">
                                @if (count($this->responsables) > 0)
                                    @foreach ($this->responsables as $index => $r)
                                        <li
                                            id="recibido-item-{{ $index }}"
                                            wire:key="recibido-{{ $r->id }}"
                                            @click="selectOption({{ $r->id }}, '{{ $r->name }} ({{ $r->cargo }})')"
                                            :class="{ 'bg-indigo-500 text-white dark:bg-indigo-600': active_index === {{ $index }}, 'text-gray-700 dark:text-gray-200': active_index !== {{ $index }} }"
                                            class="cursor-pointer px-4 py-2 text-sm transition duration-150 ease-in-out hover:bg-indigo-500 hover:text-white dark:hover:bg-indigo-600"
                                        >
            ...
            {{-- Observaciones y Detalles (el resto del formulario no se modifica) --}}
                        ...
                        @foreach ($detalles as $index => $detalle)
                            <tr wire:key="detalle-{{ $index }}" class="bg-white dark:bg-neutral-800 hover:bg-gray-50 dark:hover:bg-neutral-700">
                                <td class="px-3 py-2">
                                    ...
                                    @error("detalles.{$index}.item_tipo") <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p> @enderror
                                </td>
                                <td class="px-3 py-2">
                                    ...
                                    @error("detalles.{$index}.item_serial_placa") <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p> @enderror
                                ...
                                <td class="px-3 py-2">
                                    ...
                                    @error("detalles.{$index}.descripcion") <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p> @enderror
                                </td>
                                <td class="px-3 py-2">
                                    <input type="number" wire:model.defer="detalles.{{ $index }}.cantidad_salida" min="1" class="text-sm p-1 w-full rounded border border-gray-300 dark:border-neutral-700 bg-white dark:bg-neutral-800">
                                    @error("detalles.{$index}.cantidad_salida") <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p> @enderror
                                </td>
                                <td class="px-3 py-2">
                                    <input type="text" wire:model.defer="detalles.{{ $index }}.unidad_medida" class="text-sm p-1 w-full rounded border border-gray-300 dark:border-neutral-700 bg-white dark:bg-neutral-800">
                                </td>
            ...
            {{-- Botón Agregar Ítem --}}
            <div class="mb-6">
                <button type="button" wire:click="addDetalle" class="flex items-center px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 dark:hover:bg-blue-500 text-sm">
            ...
            {{-- Observaciones --}}
            <div class="mb-6">
                <label for="observaciones" class="block text-sm font-medium mb-1">Observaciones Generales:</label>
                <textarea id="observaciones" wire:model.defer="observaciones" rows="3" class="w-full rounded border border-gray-300 dark:border-neutral-700 focus:border-indigo-500 focus:ring focus:ring-indigo-200 dark:focus:ring-indigo-500 dark:focus:border-indigo-400 px-3 py-2 bg-white dark:bg-neutral-800 text-gray-900 dark:text-gray-100"></textarea>
                @error('observaciones') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            {{-- Botón de Guardado --}}
            <div class="flex items-center justify-end">
                <button type="submit" class="px-6 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 dark:hover:bg-indigo-500 font-bold">
